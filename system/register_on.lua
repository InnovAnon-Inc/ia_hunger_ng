...
core.register_on_item_eat(function(hpc, rwi, itemstack, user, pt)
  ...
  local weens    = hunger_def.weens    or 0
...
--  then
  ...
  then
    if (current_thirst == s.thirst.maximum and heals <= 0 and quenches >= 0) or (quenches == 0) then -- not thirsty
  ...
  then
    if (current_hunger == s.hunger.maximum and heals <= 0 and satiates >=  0) or (satiates == 0) then -- not hungry
...
f.on_joinplayer = function(player)
  ...
  if s.thirst_bar.use then -- thirst
  ...
  if s.hunger_bar.use then -- hunger
  ...
  if s.milk_bar.use then -- milk
    --minetest.log('using milk bar')
    assert(hunger_ng.milk_bar_image ~= nil)
    assert(f.get_data(player_name, a.milk_value) ~= nil)
    local milk_hud   = player:hud_add({
      type = 'statbar',
      position = { x=0.5, y=0.90 },
      text = hunger_ng.milk_bar_image,
      direction = 0,
      number = f.get_data(player_name, a.milk_value),
      size = { x=24, y=24 },
      offset = {x=25,y=-(48+24+16)},
    })
